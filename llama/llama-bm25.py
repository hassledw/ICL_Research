...
class LlamaBM25:
    ...
    def create_bm25_dict(self):
        '''
        Creates the dictionary with fewshot selections using BM25.
        '''
        corpus = [doc.split(" ") for doc in self.X_train]  # Split documents into words
        queries = [doc.split(" ") for doc in self.X_test]  # Split queries into words
        ...
        for query in queries:
            ...
            top_n = np.argsort(doc_scores)[-3:][::-1]  # Get indices of top 3 documents
            fewshot_examples.append([{'corpus_id': idx, 'score': doc_scores[idx]} for idx in top_n])
    ...
    def fewshot(self, direction, char_limit=300):
        '''
        Runs fewshot k=3 prompts on Llama model, where the fewshot examples are the most similar entries to the query using bm25.
        '''
        df = pd.DataFrame(columns=[self.X_label, self.y_label])
        count = 1
        fewshot_examples = self.create_bm25_dict()
...
{query}\nResponse: "
        ...
        df.to_csv(f"/home/grads/taylor02/ICL_Research/{self.testname}_results/{self.testname}-fewshot-llama.csv")
    ...
    def zeroshot(self, direction):
        ...
        df.to_csv(f"/home/grads/taylor02/ICL_Research/{self.testname}_results/{self.testname}-zeroshot-llama.csv")
...
if __name__ == "__main__":
    ...
    ls_yelp = LlamaBM25("UDR_Yelp", tokenizer, model)
    ...
    ls_snli = LlamaBM25("UDR_SNLI", tokenizer, model)
    direction = ""
    # ls_snli.zeroshot(direction)
    ls_snli.fewshot(direction)

    ls_come = LlamaBM25("UDR_ComE", tokenizer, model)
